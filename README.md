...
|--------|-----------|----------------|---------|
| 100 | 0.000046 с | 0.000056 с | **0.000003 с** |
| 1000 | 0.000606 с | 0.005541 с | **0.000033 с** |
| 5000 | 0.003399 с | 0.143611 с | **0.000277 с** |
| 10000 | 0.007183 с | 0.595314 с | **0.000613 с** |

**Спостереження:**

- Timsort **у 18-20 разів швидший** за Merge Sort
- Insertion Sort катастрофічно сповільнюється: у **83 рази повільніший** за Merge Sort на 10000 елементах
...
| Розмір | Merge Sort | Insertion Sort | Timsort |
|--------|-----------|----------------|---------|
| 100 | 0.000033 с | **0.000003 с** | **0.000001 с** |
| 1000 | 0.000429 с | 0.000030 с | **0.000004 с** |
| 5000 | 0.002634 с | 0.000148 с | **0.000023 с** |
| 10000 | 0.005510 с | 0.000313 с | **0.000056 с** |

**Спостереження:**

- Timsort досягає O(n) на відсортованих даних - **у 98 разів швидший** за Merge Sort!
- Insertion Sort також ефективний (O(n)) - **у 17 разів швидший** за Merge Sort
...
| Розмір | Merge Sort | Insertion Sort | Timsort |
|--------|-----------|----------------|---------|
| 100 | 0.000036 с | 0.000100 с | **0.000001 с** |
| 1000 | 0.000441 с | 0.010873 с | **0.000004 с** |
| 5000 | 0.002639 с | 0.319068 с | **0.000027 с** |
| 10000 | 0.005699 с | 1.165302 с | **0.000048 с** |

**Спостереження:**

- Insertion Sort показує найгіршу продуктивність O(n²): **у 204 рази повільніший** за Merge Sort!
- Timsort залишається найшвидшим - **у 119 разів швидший** за Merge Sort
...
| Розмір | Merge Sort | Insertion Sort | Timsort |
|--------|-----------|----------------|---------|
| 100 | 0.000038 с | 0.000014 с | **0.000002 с** |
| 1000 | 0.000511 с | 0.001409 с | **0.000025 с** |
| 5000 | 0.003012 с | 0.032255 с | **0.000150 с** |
| 10000 | 0.006621 с | 0.138223 с | **0.000310 с** |

**Спостереження:**

- Timsort найефективніший завдяки виявленню природного порядку - **у 21 раз швидший** за Merge Sort
- Insertion Sort показує прийнятні результати завдяки частковій впорядкованості
...
| Розмір | Merge Sort | Insertion Sort | Timsort |
|--------|-----------|----------------|---------|
| 50000 | 0.040745 с | 15.366295 с | **0.003624 с** |
| 100000 | 0.087981 с | 62.983695 с | **0.008170 с** |

**Спостереження:**

- Timsort **у 11 разів швидший** за Merge Sort
- Insertion Sort **у 377 разів повільніший** за Merge Sort і **у 7708 разів повільніший** за Timsort!
- На 100000 елементів: Timsort - 8 мс, Merge Sort - 88 мс, Insertion Sort - **63 секунди**!
...
- Час збільшився: 0.005541 с → 0.595314 с
- Коефіцієнт: **~107 разів** ≈ 10² ✓

При збільшенні у 100 разів (1000 → 100000):

- Час збільшився: 0.005541 с → 62.983695 с
- Коефіцієнт: **~11367 разів** ≈ 100² = 10000 ✓
...
- Час збільшився: 0.007183 с → 0.087981 с
- Коефіцієнт: **~12.2 рази**
- Теоретично: 10 × log₂(10) ≈ 10 × 3.32 ≈ **13.3** ✓

При збільшенні у 2 рази (50000 → 100000):

- Час збільшився: 0.040745 с → 0.087981 с
- Коефіцієнт: **~2.16 рази**
...
- Час: 0.000613 с → 0.008170 с
- Коефіцієнт: **~13.3 рази**
...
1. **Insertion Sort:**
   ...
   - ✅ НАЙКРАЩИЙ на вже відсортованих даних: O(n) - у 17 разів швидший за Merge Sort
   - ⚠️ Прийнятний на майже відсортованих даних малого розміру
   - ❌ Катастрофічно повільний на великих масивах: 63 секунди проти 0.088 с (Merge Sort) на 100000 елементів
...
3. **Timsort:**
   ...
   - ✅ На випадкових даних: у 11-20 разів швидший за Merge Sort
   - ✅ На відсортованих даних: у 98 разів швидший за Merge Sort
   - ✅ На зворотно відсортованих: у 119 разів швидший за Merge Sort
